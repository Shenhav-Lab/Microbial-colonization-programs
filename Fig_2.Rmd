---
title: "Nasal and gut microbiome profiles and trajectories associated with breastfeeding, asthma and other early-life factors (Figure 2)"
---
...
Overall_N_tps <- subset(Overall_NG4, SampleType == "Nasal" & Dataset != "Trajectories")
Overall_N_traj <- subset(Overall_NG4, SampleType == "Nasal" & Dataset == "Trajectories")

Overall_G_tps <- subset(Overall_NG4, SampleType == "Gut" & Dataset != "Trajectories")
Overall_G_traj <- subset(Overall_NG4, SampleType == "Gut" & Dataset == "Trajectories")
...
PLOT_pt1 <- ggplot(Overall_N_tps, aes(Variable, Variable2, fill=`Variation explained (%)`)) +
  facet_wrap(~SampleType*Dataset, ncol = 1, scales = "free_y") + geom_tile(height=0.8, width=0.8) +
...
PLOT_pt2 <- ggplot(Overall_N_traj, aes(Variable, Variable2, fill=`Variation explained (%)`)) +
  facet_wrap(~SampleType*Dataset, ncol = 1, scales = "free_y") + geom_tile(height=0.8, width=0.8) +
  scale_fill_gradient2(low="#0056cf", mid="white",  high="#d63200", midpoint = 0, limits=lims_grad, breaks=breaks_grad, labels = labels_grad) +
  geom_text(aes(label=category), size=6, nudge_y = -0.14) + 
  geom_text(aes(label=category2), size=6, nudge_y = 0.08) + 
  theme(panel.spacing.x = unit(1, "mm"), axis.title = element_blank(), axis.ticks = element_blank(),
        axis.text.x=element_blank(), 
        axis.text.y=element_text(size=12, color = "black"), strip.text.x = element_blank(), legend.position = "none", plot.margin = unit(c(t, r, 0, 0), "cm")) + 
  labs(fill = "Variation\nexplained (%)") 

PLOT_pt3 <- ggplot(Overall_G_tps, aes(Variable, Variable2, fill=`Variation explained (%)`)) +
  facet_wrap(~SampleType*Dataset, ncol = 1, scales = "free_y") + geom_tile(height=0.8, width=0.8) +
  scale_fill_gradient2(low="#0056cf", mid="white",  high="#d63200", midpoint = 0, limits=lims_grad, breaks=breaks_grad, labels = labels_grad) +
  geom_text(aes(label=category), size=6, nudge_y = -0.14) + 
  geom_text(aes(label=category2), size=6, nudge_y = 0.08) + 
...
PLOT_pt4 <- ggplot(Overall_G_traj, aes(Variable, Variable2, fill=`Variation explained (%)`)) +
  facet_wrap(~SampleType*Dataset, ncol = 1, scales = "free_y") + geom_tile(height=0.8, width=0.8) +
  scale_fill_gradient2(low="#0056cf", mid="white",  high="#d63200", midpoint = 0, limits=lims_grad, breaks=breaks_grad, labels = labels_grad) +
  geom_text(aes(label=category), size=6, nudge_y = -0.14) + 
  geom_text(aes(label=category2), size=6, nudge_y = 0.08) + 
...
# Row annotation
Var_list <- levels(Overall_NG4$Variable)
...
Var_Categories$Variable <- factor(Var_Categories$Variable, levels = levels(Overall_NG4$Variable))
...
legend_F2A <- plot_grid(get_legend(PLOT_pt1), ncol = 1)

# Removing legend from plot 1
PLOT_pt1_2 <- PLOT_pt1 + theme(legend.position = "none")

# plotting
F2A_plot <- plot_grid(PLOT_pt1_2, PLOT_pt2, PLOT_pt3, PLOT_pt4, Row_annotation, align = "v", ncol = 1, axis = "tb", rel_heights = c(15, 5.2, 15, 10, 0.9))
F2A_plot # ignore warning about missing values - these are the NA values intentionally added into dataset (for dashed cells)
```
...
```{r, include=FALSE} 
tiff(file = "Figures/Fig_2A.tiff", width = 3050, height = 2000, units = "px", res = 300)
F2A_plot
dev.off()

tiff(file = "Figures/Fig_2A_Legend.tiff", width = 500, height = 500, units = "px", res = 300)
legend_F2A
dev.off()
```


# Figure 2B-2E - Richness at single time-pointsm, Richness Trajectories, and Colonization patterns associations with breastfeeding and asthma
...
## Wilcoxon rank-sum tests (For Fig 2C, 2E and related supplementary and extra results)
...
## Wilcoxon signed-rank tests (for Figure 2B & 2D)
...
Breastfeeding, Figure 2B & 2C
```{r, warning=FALSE}
# Figure 2B Microbiota richness compared b/t timepoints, stratified
dw <- 0.65

Fig_2B <- ggplot(NG_Traj_sdf_m_Tp_BF, aes(x=BF_status_3m_sampling, y = value,
  group = interaction(Visit,BF_status_3m_sampling))) + facet_wrap(~Dataset) + geom_jitter(aes(color = interaction(Visit,Dataset)), shape=16, position=position_jitterdodge(jitter.width = 0.3, dodge.width = dw), size=1.5, alpha=0.3) +
  ...
  theme(axis.text = element_text(size=9), legend.position = "none") + labs(y="Microbiota Richness", x="Breastfeeding Exclusivity\nat 3 Months") + 
  scale_color_manual(values = Tp_Div_cols) 
Fig_2B

# needed colors to be more visible in legend, excluded alpha
#Fig_1C_Legend <- ggplot(GN_3m1y_otu_sdf2_M_BFsub, aes(x=BF_status_3m_sampling, y = value, group = interaction(variable,BF_status_3m_sampling))) + facet_wrap(~Dataset) + geom_jitter(aes(color = interaction(variable,Dataset)), shape=16, position=position_jitterdodge(jitter.width = 0.25, dodge.width = 0.75), size=3, alpha=0.75)  + scale_y_continuous(expand=c(0,0), limits = c(-0.5,300)) + theme(axis.title.x= element_blank(), axis.text = element_text(size=11), legend.position = "right") + labs(y="Microbiota Richness") + scale_color_manual(values = Tp_Div_cols) 

# Fig 2C - Richness Trajectory 
NG_Traj_sdf_m_Trajs_BF <- subset(NG_Traj_sdf_m_Trajs, BF_status_3m_sampling!="Partial") 

Fig_2C <- ggplot(NG_Traj_sdf_m_Trajs_BF, aes(x=BF_status_3m_sampling, y=value, group=BF_status_3m_sampling)) + facet_wrap(~Dataset) + 
  geom_jitter(aes(color=Dataset),shape=16, size=1.5, alpha=0.3, position = position_jitter(width = 0.15)) + geom_violin(width=0.5, trim = FALSE, alpha=0) + 
  stat_summary(fun=median, geom="point", shape=23, size=2, fill="black")  + scale_y_continuous(expand=c(0,0)) + coord_cartesian(ylim = c(-225, 200))  + 
  theme(axis.text = element_text(size=9), legend.position = "none") + labs(y="Richness Trajectory", x="Breastfeeding Exclusivity\nat 3 Months") + 
  scale_color_manual(values = Trj_cols) + geom_signif(color="black", comparisons = list(c("Exclusive", "No BM")), test = "wilcox.test", step_increase = 0.2, vjust = 2, map_signif_level = c("**"=0.001, "*"=0.051,"~"=0.1," "=2), size=0, textsize = 7) # 
Fig_2C
```

Asthma 3y, Figure 2D & 2E
```{r}
Fig_2D <- ggplot(NG_Traj_sdf_m_Tp_Ast, aes(x=asthma_3y, y = value, group = interaction(Visit,asthma_3y))) + facet_wrap(~Dataset) + 
  geom_jitter(aes(color = interaction(Visit,Dataset)), shape=16, position=position_jitterdodge(jitter.width = 0.3, dodge.width = dw), size=1.5, alpha=0.3) + 
  geom_violin(trim = FALSE, alpha=0, position=position_dodge(width = dw)) + 
  stat_summary(fun=median, geom="point", shape=23, size=2, fill="black", position=position_dodge(width = dw)) +
  scale_y_continuous(expand=c(0,0)) + 
  theme(axis.text = element_text(size=9), legend.position = "none") +
  labs(y="Microbiota Richness", x="Asthma at 3 Years") + scale_color_manual(values = Tp_Div_cols) +
  coord_cartesian(ylim = c(-10,220))
Fig_2D

NG_Traj_sdf_m_Trajs_Ast <- subset(NG_Traj_sdf_m_Trajs, asthma_3y!="Possible") 

Fig_2E <- ggplot(NG_Traj_sdf_m_Trajs_Ast, aes(x=asthma_3y, y=value, group=asthma_3y)) + facet_wrap(~Dataset) + 
  geom_jitter(aes(color=Dataset),shape=16, size=1.5, alpha=0.3, position = position_jitter(width = 0.15)) + geom_violin(width=0.5, trim = FALSE, alpha=0) + 
  stat_summary(fun=median, geom="point", shape=23, size=2, fill="black")  + scale_y_continuous(expand=c(0,0)) + coord_cartesian(ylim = c(-225, 200))  + 
  theme(axis.text = element_text(size=9), legend.position = "none") + labs(y="Richness Trajectory", x="Asthma at 3 Years") + 
  scale_color_manual(values = Trj_cols) + geom_signif(color="black", comparisons = list(c("No", "Yes")), test = "wilcox.test", step_increase = 0.2, vjust = 2, map_signif_level = c("**"=0.001, "*"=0.051,"~"=0.1," "=2), size=0, textsize = 7) # 
Fig_2E
```


```{r, warning=FALSE, include=FALSE} 
# Save Richness Plots for Figure 2

tiff(file = "Figures/Fig_2B_BF_Timepoint_RichTraj.tiff", width = 1200, height = 800, units = "px", res = 300)
Fig_2B
dev.off()
...
tiff(file = "Figures/Fig_2C_BF_RichTraj.tiff", width = 850, height = 800, units = "px", res = 300)
Fig_2C
dev.off()

tiff(file = "Figures/Fig_2D_Asthma3y_Tp_RichTraj.tiff", width = 1200, height = 800, units = "px", res = 300)
Fig_2D
dev.off()

tiff(file = "Figures/Fig_2E_Asthma3y_RichTraj.tiff", width = 850, height = 800, units = "px", res = 300)
Fig_2E
dev.off()
```
