---
title: "Figure 6. High prediction accuracy for asthma at 3 years based on nasal and gut microbiome colonization patterns and human milk components."
---
...
```{r}
Nasal_df <- read.csv(paste0(path, "Data/Data_subsets/Nasal_DivChange_ColPat_N1545_Wide_Summarized.csv"), row.names = 1)
# for nasal taxonomy
df.pa.nasal.C1 <- read.csv(paste0(path, "Tables/TableS4_Permutation_test_res_Nasal.csv"), row.names = 1)
row.names(df.pa.nasal.C1) <- df.pa.nasal.C1$OTU_Taxonomy
```
...
write.csv(Variable_Importance, paste0(path, "Tables/Variable_Importance_Nasal_trajectories.csv"))
...
write.csv(roc_curves_nasal_microbes, file = paste0(path, "Tables/roc_curves_Nasal_trajectories.csv"))

Importance_Nasal <- read.csv(file=paste0(path, "Tables/Variable_Importance_Nasal_trajectories.csv"), 
                             row.names = 1)
...
df.pa.nasal.C1_sub <- subset(df.pa.nasal.C1, select = c("Family", "Phylum", "prevalence")) # Also Add Average Prevalence's
...
Importance_Nasal_sub3$Family[is.na(Importance_Nasal_sub3$Family)] <- "_milk"
Importance_Nasal_sub3$Phylum[is.na(Importance_Nasal_sub3$Phylum)] <- "_milk" #prevalence
Importance_Nasal_sub3$prevalence[is.na(Importance_Nasal_sub3$prevalence)] <- 10

Importance_Nasal_sub4 <- head(Importance_Nasal_sub3, 20) ## changed obj name Importance_Nasal_sub4

Importance_Nasal_sub3_Nasal <- subset(Importance_Nasal_sub4, Trajectory == "Prevalence Trajectory")
...
Importance_Nasal <- subset(Importance_Nasal_sub3, Trajectory %in% c("Prevalence Trajectory", "Log2change")) ## exclude or include 'NA'? 
nrow(Importance_Nasal) ## 19

Combined_importance <- ggplot(Importance_Nasal,aes(y=reorder(OTU, Importance),x=Importance))+
  geom_point(aes(size=prevalence, color=Phylum))+theme_bw()+theme(axis.title.y =element_blank()) + 
  scale_color_manual(values = cbPalette) + labs(x="Importance")
Combined_importance ## Decision on which trajectory variable version to use? e.g. S. salivarius 5. Some kind of average?

# was originally 'Fig_5_N_Importance.tiff'
tiff(file = paste0(path, "Figures/Fig_6B_N_Importance.tiff"), width = 1500, height = 900, units = "px", res = 300) 
Combined_importance
dev.off()
```
...
View(summary(Boston.boost)) #Summary gives a table of Variable Importance and a plot of Variable Importance
Variable_Importance <- summary(Boston.boost)
write.csv(Variable_Importance, paste0(path, "Tables/Variable_Importance_Nasal_Milk_trajectories_milk_comp.csv"))
#Prediction on Test Set
##We will compute the Test Error as a function of number of Trees
n.trees = seq(from=100 ,to=10000, by=100) #no of trees-a vector of 100 values 
#Generating a Prediction matrix for each Tree
...
Importance_Nasal_Milk <- read.csv(file=paste0(path, "Tables/Variable_Importance_Nasal_Milk_trajectories_milk_comp.csv"), 
                             row.names = 1)
...
Importance_Nasal_Milk_sub3$Family[is.na(Importance_Nasal_Milk_sub3$Family)] <- "_milk"
Importance_Nasal_Milk_sub3$Phylum[is.na(Importance_Nasal_Milk_sub3$Phylum)] <- "_milk" #prevalence
...
Importance_Nasal_Milk_sub3_Nasal <- subset(Importance_Nasal_Milk_sub3, Trajectory == "Prevalence Trajectory") # should add other vars
Combined_importance_N_M <- ggplot(Importance_Nasal_Milk_sub3_Nasal,aes(y=reorder(OTU, Importance),x=Importance))+
  geom_point(aes(size=prevalence, color=Phylum))+theme_bw()+theme(axis.title.y =element_blank()) + 
  scale_color_manual(values = cbPalette) + labs(x="Importance")

tiff(file = paste0(path, "Figures/Fig_6B_N_M_Importance.tiff"), width = 1500, height = 900, units = "px", res = 300) 
Combined_importance_N_M
dev.off()
```


# Combined ROC - Nasal with and without milk components, trajectories and timepoints
...
# was originally Fig_5_N_ROC_plot.tiff
tiff(file = paste0(path, "Figures/Fig_6A_N_ROC_plot.tiff"), width = 1500, height = 900, units = "px", res = 300) 
Nasal_ROC_plot
dev.off()
```
...
```{r}
Gut_df <- read.csv(paste0(path, "Data/Data_subsets/Gut_DivChange_ColPat_N555_Wide_Summarized.csv"), row.names = 1)
# for gut taxonomy
df.pa.gut.C1 <- read.csv(paste0(path, "Tables/TableS4_Permutation_test_res_Gut.csv"), row.names = 1)
row.names(df.pa.gut.C1) <- df.pa.gut.C1$OTU_Taxonomy
```
...
features_to_remove<- which(colnames(Gut_df)[feature_ids] %in%  c("CHILDid", "asthma_3y", "asthma_5y", "atopy3y", "atopy5y", "AR_posprob_5y","AR_posprob_3y", "AR_5y", "StudyCenter", "recwheeze5y", "asthma_posprob_3y", 
 "asthma_posprob_5y", "AD_posprob_3y", "AD_posprob_5y", "recwheeze1y", "atopy1y", "wheeze3m", "mom_asthma_ever",
 "Earinf_1y", "Fever_3m", "birthmode2", "Fever_1y", "weight_0", "age_3m_months", "mom_age_birth")) 
## Note - added removal of: "weight_0", "age_3m_months", "mom_age_birth", to do: remove Shannon_G1y, Observed_G1y, Shannon_G1y3m, etc? (in updated input df)
...
View(summary(Boston.boost)) #Summary gives a table of Variable Importance and a plot of Variable Importance
Variable_Importance <- summary(Boston.boost)
write.csv(Variable_Importance, paste0(path, "Tables/Variable_Importance_Gut_trajectories.csv"))
#Prediction on Test Set
##We will compute the Test Error as a function of number of Trees
n.trees = seq(from=100 ,to=10000, by=100) #no of trees-a vector of 100 values 
#Generating a Prediction matrix for each Tree
...
write.csv(roc_curves_gut_microbes, file = paste0(path, "Tables/roc_curves_Gut_trajectories.csv"))

Importance_Gut <- read.csv(file=paste0(path, "Tables/Variable_Importance_Gut_trajectories.csv"), 
                             row.names = 1)
...
df.pa.gut.C1_sub <- subset(df.pa.gut.C1, select = c("Family", "Phylum", "prevalence")) # Also Add Average Prevalence's
...
Importance_Gut_sub3$Family[is.na(Importance_Gut_sub3$Family)] <- "_milk"
Importance_Gut_sub3$Phylum[is.na(Importance_Gut_sub3$Phylum)] <- "_milk" #prevalence
Importance_Gut_sub3$prevalence[is.na(Importance_Gut_sub3$prevalence)] <- 10

Importance_Gut_sub4 <- head(Importance_Gut_sub3, 20) ## renamed obj. Importance_Gut_sub4

Importance_Gut_sub3_Gut <- subset(Importance_Gut_sub4, Trajectory == "Prevalence Trajectory")
Combined_importance_Prev <- ggplot(Importance_Gut_sub3_Gut,aes(y=reorder(OTU, Importance),x=Importance))+
  geom_point(aes(size=prevalence, color=Phylum))+theme_bw()+theme(axis.title.y =element_blank()) + 
  scale_color_manual(values = cbPalette) + labs(x="Importance")

Importance_Gut_0 <- subset(Importance_Gut_sub3, Trajectory %in% c("Prevalence Trajectory", "Log2change")) ## exclude or include 'NA'? 
nrow(Importance_Gut_0) 

Combined_importance <- ggplot(Importance_Gut_0,aes(y=reorder(OTU, Importance),x=Importance))+
  geom_point(aes(size=prevalence, color=Phylum))+theme_bw()+theme(axis.title.y =element_blank()) + 
  scale_color_manual(values = cbPalette) + labs(x="Importance")
Combined_importance 

## was originally 'Fig_5_G_Importance.tiff'
tiff(file = paste0(path, "Figures/Fig_6C_G_Importance.tiff"), width = 1500, height = 900, units = "px", res = 300) 
Combined_importance
dev.off()
```
...
features_to_remove<- which(colnames(Gut_df)[feature_ids] %in%  c("CHILDid", "asthma_3y", "asthma_5y", "atopy3y", "atopy5y", "AR_posprob_5y","AR_posprob_3y", "AR_5y", "StudyCenter", "recwheeze5y", "asthma_posprob_3y", 
 "asthma_posprob_5y", "AD_posprob_3y", "AD_posprob_5y", "recwheeze1y", "atopy1y", "wheeze3m", "mom_asthma_ever",
 "Earinf_1y", "Fever_3m", "birthmode2", "Fever_1y", "mom_age_birth", "age_3m_months", "weight_0"))
...
View(summary(Boston.boost)) #Summary gives a table of Variable Importance and a plot of Variable Importance
Variable_Importance <- summary(Boston.boost)
write.csv(Variable_Importance, paste0(path, "Tables/Variable_Importance_Gut_Milk_trajectories_milk_comp.csv"))
#Prediction on Test Set
##We will compute the Test Error as a function of number of Trees
n.trees = seq(from=100 ,to=10000, by=100) #no of trees-a vector of 100 values 
#Generating a Prediction matrix for each Tree
...
Importance_Gut_Milk <- read.csv(file=paste0(path, "Tables/Variable_Importance_Gut_Milk_trajectories_milk_comp.csv"), 
                             row.names = 1)
...
Importance_Gut_Milk_sub3$Family[is.na(Importance_Gut_Milk_sub3$Family)] <- "_milk"
Importance_Gut_Milk_sub3$Phylum[is.na(Importance_Gut_Milk_sub3$Phylum)] <- "_milk" #prevalence
...
Importance_Gut_Milk_sub3_Gut <- subset(Importance_Gut_Milk_sub3, Trajectory == "Prevalence Trajectory") # should add other vars
Combined_importance_G_M <- ggplot(Importance_Gut_Milk_sub3_Gut,aes(y=reorder(OTU, Importance),x=Importance))+
  geom_point(aes(size=prevalence, color=Phylum))+theme_bw()+theme(axis.title.y =element_blank()) + 
  scale_color_manual(values = cbPalette) + labs(x="Importance")

tiff(file = paste0(path, "Figures/Fig_6C_G_M_Importance.tiff"), width = 1500, height = 900, units = "px", res = 300) 
Combined_importance_G_M
dev.off()
```


# Combined ROC - Gut with and without milk components, trajectories and timepoints
...
Gut_ROC_plot <- ggplot() +
  ...
  geom_line(aes(x,x), size = 0.2) + theme_bw() + labs(x = "False Positive fraction",
                                                      y = "True Positive fraction") + theme_bw(base_size=12)
# was originally 'Fig_5_G_ROC_plot.tiff'
tiff(file = paste0(path, "Figures/Fig_6A_G_ROC_plot.tiff"), width = 1500, height = 900, units = "px", res = 300) 
Gut_ROC_plot
dev.off()
```
