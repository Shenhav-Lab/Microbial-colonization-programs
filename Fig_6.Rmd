---
title: "Colonization patterns in the nasal and gut microbiota mediate the association between breastfeeding exclusivity and asthma (Figure 6)"
---
...
## NASAL-GUT Multiple Mediation Model (Figure 6)
**Model**
```{r}
NG_Mediation_Mod <- ' 
    asthma_3y ~ b1 * micro_N + b2 * micro_G + c1 * BF_status_3m_sampl_num + c2 * coldsnum_0to3m + c3 * older_sibs_living + c4 * atbx_bin + c5 * prenatal_smoke_num + c6 * birthmode2_num + c7 * mom_asthma_num + c8 * Toronto  +  c9 * RE_Virus_1y_num ## Direct effects on asthma outcome have path c labels; effects of the nasal mediator has path b1 label and gut mediator has path b2 label
    
    micro_N ~  a1 * BF_status_3m_sampl_num + d2 * coldsnum_0to3m + d3 * older_sibs_living + d4 * atbx_bin + d5 * prenatal_smoke_num + d6 * birthmode2_num + d7 * mom_asthma_num + d8 * Toronto + d9 * RE_Virus_1y_num ## Main exogenous factor (BF) associated with Nasal microbiota has path a1 label; exogenous covariates associated with nasal microbiota have path d label
    
    micro_G ~ a2 * BF_status_3m_sampl_num + e2 * coldsnum_0to3m + e3 * older_sibs_living + e4 * atbx_bin + e5 * prenatal_smoke_num + e6 * birthmode2_num + e7 * mom_asthma_num + e8 * Toronto + e9 * RE_Virus_1y_num ## Main exogenous factor (BF) associated with Gut microbiota has path a2 label; exogenous covariates associated with Gut microbiota have path e labels
