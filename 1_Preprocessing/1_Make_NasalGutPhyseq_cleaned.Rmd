...
```{r}
#OTU table
OTU_table <- read.table(paste0(path, "Data/Preprocessing/nasal_input/feature-table-tax_nasal_March2021.txt"),
                        header=T, sep="\t", na.strings="NA", dec=".", strip.white=TRUE, row.names =1)
...
N_sdf_path <- "Data/Preprocessing/nasal_input/metadata/CSswabs_OnlySamples_nov2020_KF.csv"

### IDs AND RUN NUMBERS we have fastq files for
N_IDs_path <- "Data/Preprocessing/nasal_input/metadata/New_CS_SampleIDs_Mar_2021.csv"

### Control annotations 
N_cont_path <- "Data/Preprocessing/nasal_input/metadata/Control_List_Mar2021_LR_KF_UPDATED.csv"


## read in inputs

nasal_metadata <- read.csv(paste0(path, N_sdf_path))
dim(nasal_metadata) 
### remove unnecessary cols
nasal_metadata$Addition <- NULL

nasal_Ids <- read.csv(paste0(path, N_IDs_path), row.names = 1)
dim(nasal_Ids) 
colnames(nasal_Ids) <- c("SeqID", "RunID")

control_annotate <- read.csv(paste0(path, N_cont_path))
``` 
...
write.csv(nasal_metadata_missing, file=paste0(path, "Data/Preprocessing/output/nasal_LabData_missing_fastq_Mar2021.csv"))
...
# Input
Original_viral_path <- "Data/Preprocessing/nasal_input/metadata/2_02 - Current specimen ID and virus 06Mar2020.csv"
New_viral_path <- "Data/Preprocessing/nasal_input/metadata/1_CHILD Viral Data vMay2020.csv"
All_N_path <- "Data/Preprocessing/nasal_input/metadata/EGCD ALL Nasal Samples_V11Aug2020.csv"


#Old and new viral data was provided, and a list of all samples
All_samples_old <- read.csv(paste0(path, Original_viral_path)) # differentiates viral and nasal swabs

All_samples <- read.csv(paste0(path, New_viral_path))

Nasal_ALL_new <- read.csv(paste0(path, All_N_path))
```
...
write.csv(nasal_all_samples, file=paste0(path, "Data/Preprocessing/output/EGCD_Viral_nasal_data_Compiled_06122_KF.csv"))
```
...
PFT_ls_path <- "Data/Preprocessing/nasal_input/metadata/Subjects with nasal m and PFT.csv"

PFT_data <- read.csv(paste0(path, PFT_ls_path))
...
write.csv(nasal_metadata3, file=paste0(path, "Data/Preprocessing/output/Samples_for_nasal_withVirus_060321.csv"))
```
...
saveRDS(physeq, file=paste0(path, 'Data/Preprocessing/output/datasets/physeq_nasal_Closed.rds'))
```
...
```{r, include=FALSE}
saveRDS(physeq_child, file=paste0(path, 'Data/Preprocessing/output/datasets/phy_nasal_Close_CHILD_March2021.rds'))
```
...
```{r}
#OTU table
OTU_table <- read.table(file=paste0(path, "Data/Preprocessing/gut_input/gut-feature-table-tax.txt"),
                        header=T, sep="\t", na.strings="NA", dec=".", strip.white=TRUE, row.names =1)
...
```{r}
# Inputs
gut_sdf_path <- "Data/Preprocessing/gut_input/metadata/Gut_Metadata_subjects_matched_shirin_July19_2.txt" # gut sample lab metadata
gut_Runs_path <- "Data/Preprocessing/gut_input/metadata/Stool_IDs_RunID_List.csv" # samples w/t sequencing data
Blank_sdf_path <- "Data/Preprocessing/gut_input/metadata/Blanks_confirmed_per_run.csv" # Blank - RunID link - identified previously based on a master sequencing sheet and the 'location' of each blank ("location" in file names could be matched to master sequencing sheet)
Blank_name_path <- "Data/Preprocessing/gut_input/metadata/BlankNameChange_Jan2019.txt" # blank names were relabeled at some point for demultiplexing (not by me). This is a list of fastq file names for blanks 
Blank_files_path <- "Data/Preprocessing/gut_input/metadata/MANIFEST_Blanks" # name of files sent to me originally for blanks

## any sampleIDs not in lab metadata are not to be analyzed (12 samples)
gut_metadata <- read.table(paste0(path, gut_sdf_path), header=TRUE, sep="\t", na.strings="NA", dec=".", strip.white=TRUE)

RunIDs <- read.csv(paste0(path, gut_Runs_path))
colnames(RunIDs) <- c("SampleID", "RunID") ##  re-label colnames

Blank_matching <- read.csv(paste0(path, Blank_sdf_path))
...
Blank_name_changes <- read.table(paste0(path, Blank_name_path), header=FALSE) 
...
Blank_filenames <- read.csv(paste0(path, Blank_files_path))
```
...
```{r, include=FALSE}
saveRDS(physeq_gut, file=paste0(path, 'Data/Preprocessing/output/datasets/physeq_gut_Closed.rds'))
```
...
```{r, include=FALSE}
saveRDS(gut_noMissing, file=paste0(path, 'Data/Preprocessing/output/datasets/physeq_gut_Closed_CHILD.rds'))
```
