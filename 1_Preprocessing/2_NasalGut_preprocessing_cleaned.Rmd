...
```{r}
physeq_child <- readRDS(file=paste0(path, 'Data/Preprocessing/output/datasets/phy_nasal_Close_CHILD_March2021.rds'))
``` 
...
```{r, include=FALSE}
saveRDS(physeq_bac, file=paste0(path, 'Data/Preprocessing/output/datasets/physeq_nasal_bacteria.rds'))
```
...
write.csv(depth_PerRun, file=paste0(path, "Data/Preprocessing/output/Nasal_preDeconReadCounts_032021.csv"), quote=FALSE)
depth_PerRun
```
...
write.csv(df.pa, file=paste0(path, "Data/Preprocessing/output/Nasal_Contaminant_InFo.csv"), quote = FALSE)
```
...
#saveRDS(Py_nonAllcontam, file=paste0(path, 'Data/Preprocessing/output/datasets/N_phyloseq_postDecontam.rds'))
```
...
write.csv(depth_PerRun, file=paste0(path, "Data/Preprocessing/output/N_postDeconReadCounts.csv"), quote=FALSE)
...
# not evaluating this chunk when knitting html, takes too long
### Rarefaction slopes, are new OTUs being found or is richness 'saturated' at 8000 reads?
# splitting by visit so that lines are actually visible
#  just for plot, only include samples
ForPlot <- subset_samples(Py_nonAllcontam, SampleType == "Sample" & Visit == "3 Months") %>% prune_taxa(taxa_sums(.)>0, .) %>% otu_table %>% as.data.frame %>% t # have to make df before t otherwise logical subscript too long error
...
set.seed(9999)
summary(rareslope(ForPlot, sample=8000)) # selected option

summary(rareslope(ForPlot, sample=10000)) 

set.seed(9999)

tiff(file = "rarecurve_nasal_3m.tiff", width = 1500, height = 2000, units = "px", res = 300)
rarecurve(ForPlot, step = 1000, cex=0.5, label = FALSE, xlim = c(1000, 50000)) # step 5000 to increase speed of comp
abline(v = 8000, col="blue")
##abline(v = 10000, col="orange")
dev.off()

ForPlot <- subset_samples(Py_nonAllcontam, SampleType == "Sample" & Visit == "1 Year") %>% prune_taxa(taxa_sums(.)>0, .) %>% otu_table %>% as.data.frame %>% t # have to make df before t otherwise logical subscript too long error

# rarefaction curve takes forever, below is a quicker check for re-running code in the future - just to check the slope values rather than visualizing entire plot
set.seed(9999)
summary(rareslope(ForPlot, sample=8000)) # selected option- median = 0.0009654, IQR = 0.0003526 - 0.0017624

summary(rareslope(ForPlot, sample=10000)) # median  = 0.0006471, IQR = 0.0002157 - 0.0012645

summary(rareslope(ForPlot, sample=25000)) # 0.000088418 (0.000004626 - 0.000004626)

set.seed(9999)

tiff(file = "rarecurve_nasal_1y.tiff", width = 1500, height = 2000, units = "px", res = 300)
rarecurve(ForPlot, step = 1000, cex=0.5, label = FALSE, xlim = c(1000, 50000)) # step 5000 to increase speed of comp
abline(v = 8000, col="blue")
##abline(v = 10000, col="orange")
dev.off()

ForPlot <- subset_samples(Py_nonAllcontam, SampleType == "Sample") %>% prune_taxa(taxa_sums(.)>0, .) %>% otu_table %>% as.data.frame %>% t # have to make df before t otherwise logical subscript too long error

# rarefaction curve takes forever, below is a quicker check for re-running code in the future - just to check the slope values rather than visualizing entire plot
set.seed(9999)
summary(rareslope(ForPlot, sample=8000)) 
...
write.csv(taxa_abund_UnRare2, file=paste0(path, "Data/Preprocessing/output/Nasal_Unrarified_taxa_abundances.csv"), quote = FALSE)
```
...
write.csv(depth_PerRun, file=paste0(path, "Data/Preprocessing/output/Nasal_PostSampleAndRareTaxaFilt_Counts.csv"), quote=FALSE)
```
...
saveRDS(UnRarefyCount_prune, file=paste0(path, "Data/Preprocessing/output/datasets/Nasal_UnRarefied_Pruned_Py.rds"))
```
...
```{r, include=FALSE}
saveRDS(UnRarefyCount_prune2, file=paste0(path, "Data/Preprocessing/final_datasets/01_Nasal_UnRarefied_Py.rds"))
```
...
```{r, include=FALSE}
# Save Physeq Obj
saveRDS(physeq_bac_G, file=paste0(path, 'Data/Preprocessing/output/datasets/physeq_gut_bacteria'))
```
...
write.csv(depth_PerRun, file=paste0(path, "Data/Preprocessing/output/Gut_preDeconReadCounts_052021.csv"), quote=FALSE)
```
...
# Not running in knit b/c takes too long to generate plots. 
### Rarefaction slopes, are new OTUs being found or is richness 'saturated' at 8000 reads?
# splitting by visit so that lines are actually visible
#  just for plot, only include samples
ForPlot <- subset_samples(physeq_bac_G, SampleType == "Gut" & Visit == "3 Months") %>% prune_taxa(taxa_sums(.)>0, .) %>% otu_table %>% as.data.frame %>% t # have to make df before t otherwise logical subscript too long error

set.seed(9999)

tiff(file = paste0(path, "Figures/Fig_S10_rarecurve_gut_3m.tiff"), width = 1500, height = 2000, units = "px", res = 300)
rarecurve(ForPlot, step = 1000, cex=0.5, label = FALSE, xlim = c(1000, 50000)) # step 5000 to increase speed of comp
abline(v = 8000, col="blue")
##abline(v = 10000, col="orange")
dev.off()

ForPlot <- subset_samples(physeq_bac_G, SampleType == "Gut" & Visit == "1 Year") %>% prune_taxa(taxa_sums(.)>0, .) %>% otu_table %>% as.data.frame %>% t # have to make df before t otherwise logical subscript too long error

set.seed(9999)

tiff(file = paste0(path, "Figures/Fig_S10_rarecurve_gut_1y.tiff"), width = 1500, height = 2000, units = "px", res = 300)
rarecurve(ForPlot, step = 1000, cex=0.5, label = FALSE, xlim = c(1000, 50000)) # step 5000 to increase speed of comp
abline(v = 8000, col="blue")
##abline(v = 10000, col="orange")
dev.off()

ForPlot <- subset_samples(physeq_bac_G, SampleType == "Gut") %>% prune_taxa(taxa_sums(.)>0, .) %>% otu_table %>% as.data.frame %>% t 
...
summary(rareslope(ForPlot, sample=10000)) # other possible option
...
write.csv(taxa_abund_UnRare2, file=paste0(path, "Data/Preprocessing/output/Gut_Unrarified_taxa_abundances.csv"), quote = FALSE)
```
...
write.csv(depth_PerRun, file=paste0(path, "output/Gut_PostSampleAndRareTaxaFilt_Counts.csv"), quote=FALSE)
```
...
```{r, include=FALSE}
##Save Pruned/Un-Rarified version
saveRDS(UnRarefyCount_prune_G, file=paste0(path, "Data/Preprocessing/final_datasets/UnRarefied_Pruned_Py_gut.rds"))
```
...
saveRDS(as(sample_data(physeq), "data.frame"), file=paste0(path, "Data/Preprocessing/final_datasets/nasal_milk_gut_sample_data"))
```
...
# Saved datasets
```{r}
saveRDS(physeq, file=paste0(path, "Data/Preprocessing/final_datasets/02_UnRarefied_Merged_Milk_Nasal_Gut.rds"))

sdf <- as(sample_data(physeq), "data.frame")
# Save sample metadata separately too
write.csv(sdf, file=paste0(path, "Data/Preprocessing/final_datasets/Milk_Nasal_Gut_merged_sample_data.csv"))

saveRDS(physeq_rarif, file=paste0(path, "Data/Preprocessing/final_datasets/02_Rarefied_Merged_Milk_Nasal_Gut.rds"))
```
