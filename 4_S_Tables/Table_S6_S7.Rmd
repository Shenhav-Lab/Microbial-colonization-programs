---
title: "Table S6 & Table S7. Multivariable mediation model estimates, standard errors and p-values for all paths (S6). Fit statistics for the combined nasal-gut microbiota multiple mediation model, and individual mediation models for nasal and gut microbiota (S7)"
---
...
## Nasal-Gut Multiple Mediation Model
...
## Nasal Mediation Model
**Model**
```{r}
N_Mediation_Mod <- ' 
    asthma_3y ~ b1 * micro_N + c1 * BF_status_3m_sampl_num + c2 * coldsnum_0to3m + c3 * older_sibs_living + c4 * atbx_bin + c5 * prenatal_smoke_num + c6 * birthmode2_num + c7 * mom_asthma_num + c8 * Toronto  +  c9 * RE_Virus_1y_num + c10 * age_3m_months
...
```{r}
G_Mediation_Mod <- ' 
    asthma_3y ~ b1 * micro_G + c1 * BF_status_3m_sampl_num + c2 * coldsnum_0to3m + c3 * older_sibs_living + c4 * atbx_bin + c5 * prenatal_smoke_num + c6 * birthmode2_num + c7 * mom_asthma_num + c8 * Toronto +  c9 * RE_Virus_1y_num + c10 * age_3m_months
...
## Table S5 - Combined Results (these results are also used to generate Figure 6 & Figure S7)
```{r}
...
write.csv(Combined_SEM_models_V3, file=paste0(path, "Tables/TableS6_SEM_Model_Res_Fig6_FigS7_V03_p05.csv"))
```


## Sensitivity analysis - Same sample size (and same set of participants) for separate nasal and gut models
Using the paired data subset - same dataset used for the combined nasal-gut model

### NASAL Mediation Model (n = 341)
**Run Nasal Model on paired dataset**
```{r}
set.seed(38974)

Med_N_2 <- sem(model = paste(micro.model_N, "\n", N_Mediation_Mod),
            data = NG_3m1y_asthm_traj_sdf_noNA, ordered = c("asthma_3y")) 

summary(Med_N_2, fit.measures = TRUE)
```

**Get standardized estimates and other Model results**
```{r}
Med_N_stdEst_2 <- parameterEstimates(Med_N_2, boot.ci.type = "bca.simple", level = .95, ci = TRUE, standardized = TRUE)
kable(Med_N_stdEst_2)
```

### GUT Mediation Model (n = 341) 
**Run Gut Model on paired dataset**
```{r}
set.seed(38974)

Med_G_2 <- sem(model = paste(micro.model_G, "\n", G_Mediation_Mod),
            data = NG_3m1y_asthm_traj_sdf_noNA, ordered = c("asthma_3y")) 

summary(Med_G_2, fit.measures = TRUE)
```

**Get standardized estimates and other Model results**
```{r}
Med_G_stdEst_2 <- parameterEstimates(Med_G_2, boot.ci.type = "bca.simple", level = .95, ci = TRUE, standardized = TRUE)
kable(Med_G_stdEst_2)
```
