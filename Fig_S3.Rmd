---
title: "Breastfeeding and asthma are associated with similar colonization patterns (Figure S3)"
---
...
### Colonization patterns (Figure S3)
The proportion of total taxa that are early, persistent and late colonizers per-infant
```{r} 
## edit variable names for plot
NG_Traj_sdf_m_ColPats$variable <- gsub("_.*$", "", NG_Traj_sdf_m_ColPats$variable)
NG_Traj_sdf_m_ColPats$variable <- factor(NG_Traj_sdf_m_ColPats$variable, levels = c("Early", "Persistent", "Late"))

BF_ColCounts <- subset(NG_Traj_sdf_m_ColPats, BF_status_3m_sampling != "Partial")

Ast_ColCounts <- subset(NG_Traj_sdf_m_ColPats, asthma_3y != "Possible")
```

```{r, warning=FALSE, fig.width=9}
Fig_S3AB_BF <- ggplot(BF_ColCounts, aes(x=BF_status_3m_sampling, y=value, group=BF_status_3m_sampling)) + 
  facet_wrap(~Dataset*variable, ncol=6) + 
  ...
  labs(y="Percent of OTUs", x="Breastfeeding Exclusivity at 3 Months") + 
...
Fig_S3 <- plot_grid(Fig_S3AB_BF, Fig_S3AB_Ast, nrow = 2, axis = "tb")
Fig_S3
```

```{r, include=FALSE}
tiff(file = "figures/FigS3_All_ColPatterns_Final.tiff", width = 2600, height = 1350, units = "px", res = 300)
Fig_S3
dev.off()
```
